#!/bin/bash
# Phase 2 Chain A-3: spatial_pca_multi_goal + delta patch weights
# Chain A (GPU 2,3,4): 1 → 3 → 5
# waits for chain3 eval (eval_spatial_pca_multi_goal_attn_best.log)
...
log() { echo "[CHAIN5_GPU234 $(date '+%H:%M:%S')] $*"; }

log "Waiting for chain3 eval (eval_spatial_pca_multi_goal_attn_best.log) ..."
until grep -l "Success Rate" "$SWM/logs/eval_spatial_pca_multi_goal_attn_best.log" 2>/dev/null | grep -q .; do
    sleep 60
done
log "chain3 done. Starting spatial_pca_multi_goal_delta on GPU 2,3,4 ..."

TRAIN_LOG="$SWM/logs/train_spatial_pca_multi_goal_delta_$(date +%Y%m%d_%H%M%S).log"
( cd $SWM && CUDA_VISIBLE_DEVICES=2,3,4 $WMPO_TORCHRUN \
    --nproc_per_node=3 --master_port=29554 \
    "$SWM/scripts/train_stage3.py" \
    --config "$SWM/configs/stage3_spatial_pca_multi_goal_delta.yaml" \
    > "$TRAIN_LOG" 2>&1 )
log "Training DONE. Log: $TRAIN_LOG"

OUT="$SWM/outputs/stage3/spatial_pca_multi_goal_delta/square"
