...
log() { echo "[CHAIN10_GPU567 $(date '+%H:%M:%S')] $*"; }

log "Starting pca_binary_dino_attn on GPU 5,6,7 ..."
TRAIN_LOG="$SWM/logs/train_spatial_pca_binary_dino_attn_$(date +%Y%m%d_%H%M%S).log"
( cd $SWM && CUDA_VISIBLE_DEVICES=5,6,7 $WMPO_TORCHRUN \
    --nproc_per_node=3 --master_port=29559 \
    "$SWM/scripts/train_stage3.py" \
    --config "$SWM/configs/stage3_spatial_pca_binary_dino_attn.yaml" \
    > "$TRAIN_LOG" 2>&1 )
log "Training DONE. Log: $TRAIN_LOG"

OUT="$SWM/outputs/stage3/spatial_pca_binary_dino_attn/square"
...
SAVE_JSON="$SWM/eval_results/spatial_pca_binary_dino_attn_best_$(date +%Y%m%d_%H%M%S).json" \
$WMPO_PYTHON -u "$SWM/eval/eval_swm_mimicgen.py" \
> "$SWM/logs/eval_spatial_pca_binary_dino_attn_best.log" 2>&1 &
P1=$!
...
VLA_BASE="/NHNHOME/WORKSPACE/0526040052_A/sjLee/WMPO/checkpoint_files/SFT_models/square" \
VLA_DEVICE=cuda N_EPISODES=50 \
SAVE_JSON="$SWM/eval_results/spatial_pca_binary_dino_attn_last_$(date +%Y%m%d_%H%M%S).json" \
$WMPO_PYTHON -u "$SWM/eval/eval_swm_mimicgen.py" \
> "$SWM/logs/eval_spatial_pca_binary_dino_attn_last.log" 2>&1 &
P2=$!

wait $P1 $P2
log "Eval DONE."
grep -h "Success Rate" \
    "$SWM/logs/eval_spatial_pca_binary_dino_attn_best.log" \
    "$SWM/logs/eval_spatial_pca_binary_dino_attn_last.log" 2>/dev/null | sed 's/^/  /'
log "chain10 ALL DONE."
