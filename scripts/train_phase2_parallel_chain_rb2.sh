#!/bin/bash
# Phase 2 병렬 chain (robust_b2): 실험 2개씩 동시 학습 + 4개 eval 동시 실행
#
# 구조 (한 쌍당):
...
STAGE2_CKPT="${SWM}/outputs/stage2/robust_b2/best.pt"
N_EPISODES=50
N_GPUS=2
...
while [ $i -lt $TOTAL ]; do
    ...
    CFG_A="${SWM}/configs/robust_b2/${EXP_A}.yaml"
    LOG_A="${SWM}/logs/rb2_${EXP_A}_${TS}.log"

    echo ""
    if [ $HAS_B -eq 1 ]; then
        CFG_B="${SWM}/configs/robust_b2/${EXP_B}.yaml"
        LOG_B="${SWM}/logs/rb2_${EXP_B}_${TS}.log"
    ...
    JSON_A_BEST="${SWM}/eval_results/rb2_${EXP_A}_best_${TS}.json"
    JSON_A_LAST="${SWM}/eval_results/rb2_${EXP_A}_last_${TS}.json"

    echo "  Eval 시작..."
    if [ $HAS_B -eq 1 ]; then
        JSON_B_BEST="${SWM}/eval_results/rb2_${EXP_B}_best_${TS}.json"
        JSON_B_LAST="${SWM}/eval_results/rb2_${EXP_B}_last_${TS}.json"
