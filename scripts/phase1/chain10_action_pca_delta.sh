...
log() { echo "[CHAIN10 $(date '+%H:%M:%S')] $*"; }

log "Waiting for eval_pca_max_best.log ..."
until grep -l "Success Rate" "$SWM/logs/eval_pca_max_best.log" 2>/dev/null | grep -q .; do
    sleep 60
done
log "Dependency DONE. Starting action_pca_delta training ..."

TRAIN_LOG="$SWM/logs/train_action_pca_delta_$(date +%Y%m%d_%H%M%S).log"
cd $SWM && CUDA_VISIBLE_DEVICES=2,3,7 $WMPO_TORCHRUN --nproc_per_node=3 --master_port=29515 \
