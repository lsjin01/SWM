...
log "Dependency DONE. Starting var_pool_binary training ..."

TRAIN_LOG="$SWM/logs/train_var_pool_binary_$(date +%Y%m%d_%H%M%S).log"
cd $SWM && CUDA_VISIBLE_DEVICES=2,3,7 $WMPO_TORCHRUN --nproc_per_node=3 --master_port=29530 \
    "$SWM/scripts/train_stage3.py" \
    --config "$SWM/configs/stage3_var_pool_binary.yaml" \
    > "$TRAIN_LOG" 2>&1
log "var_pool_binary training DONE."

OUT="$SWM/outputs/stage3/var_pool_binary/square"
log "Evaluating var_pool_binary best(GPU2) + last(GPU3) ..."
...
SAVE_JSON="$SWM/eval_results/var_pool_binary_best_$(date +%Y%m%d_%H%M%S).json" \
$WMPO_PYTHON "$SWM/eval/eval_swm_mimicgen.py" > "$SWM/logs/eval_var_pool_binary_best.log" 2>&1 &
PID1=$!
...
VLA_BASE="/NHNHOME/WORKSPACE/0526040052_A/sjLee/WMPO/checkpoint_files/SFT_models/square" \
VLA_DEVICE=cuda N_EPISODES=50 \
SAVE_JSON="$SWM/eval_results/var_pool_binary_last_$(date +%Y%m%d_%H%M%S).json" \
$WMPO_PYTHON "$SWM/eval/eval_swm_mimicgen.py" > "$SWM/logs/eval_var_pool_binary_last.log" 2>&1 &
PID2=$!

wait $PID1 $PID2
log "var_pool_binary eval DONE."
grep -h "Success Rate" "$SWM/logs/eval_var_pool_binary_best.log" "$SWM/logs/eval_var_pool_binary_last.log" 2>/dev/null | sed 's/^/  /'
