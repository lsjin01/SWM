...
mkdir -p "$LOGDIR"
exec 1>"$LOGDIR/eval_official_${SLURM_JOB_ID}.log"
exec 2>&1
...
echo "=== Official eval.py method (predict_action): $(date) / Node: $(hostname) ==="
...
run_eval() {
    ...
    local VLA_BASE=$2
    local GPU=$3
    local SAVE_JSON=$SWM/eval_results/official_${TASK}_${MODEL_NAME}.json
...
export SWM_ROOT=$SWM
export PYTHONPATH=$SWM/dependencies/openvla-oft:$SWM/dependencies/openvla-oft/experiments/robot:$SWM/dependencies/WMPO/dependencies/openvla-oft:$SWM/dependencies/WMPO/dependencies/openvla-oft/experiments/robot:\$PYTHONPATH

python3 $SWM/eval/eval_official.py \
    --task $TASK \
    --ckpt $VLA_BASE \
    ...
    --gpu 0 \
    --save_json $SAVE_JSON \
    2>&1 | tee $LOGDIR/eval_official_${TASK}_${MODEL_NAME}_${SLURM_JOB_ID}.log
"
}
...
echo ""
echo "====== OFFICIAL EVAL RESULTS ($TASK, $N_EPISODES episodes) ======"
echo "  Method: predict_action (official eval.py)"
for MODEL in SFT WMPO_P128 WMPO_P1280; do
    JSON=$SWM/eval_results/official_${TASK}_${MODEL}.json
    if [ -f "$JSON" ]; then
        python3 -c "
import json
